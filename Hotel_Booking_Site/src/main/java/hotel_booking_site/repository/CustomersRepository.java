...
public interface CustomersRepository extends JpaRepository<Customer, Long> {
	...
	@Query(value="SELECT c.id, c.first_name, c.last_name, c.email, c.password, c.current_balance FROM customers c WHERE c.email = ?1 AND c.password = ?2 LIMIT 1", nativeQuery=true)
